...
class NegotiateAuthentication extends AuthenticationInfo
{
	...
	/**
	 * Constructor used for WWW entries. <code>pw</code> is not used because for
	 * GSS there is only one single PasswordAuthentication which is independant
	 * of host/port/... info.
	 */
	...
	/**
	 * Find out if a hostname supports Negotiate protocol. In order to find out
	 * yes or no, an initialization of a Negotiator object against hostname and
	 * scheme is tried. The generated object will be cached under the name of
	 ...
	 */
	public static synchronized boolean isSupported(String hostname,
			String scheme)
	{
		if (supported == null)
		{
		...
		try
		{
			final Negotiator neg = Negotiator.getSupported(hostname, scheme);
	 ...
	 * @param p
	 *            A source of header values for this connection, not used
	 *            because HeaderParser converts the fields to lower case, use
	 *            raw instead
	 * @param raw
	 ...
	 */
	@Override
	synchronized boolean setHeaders(final HttpURLConnection conn,
			final HeaderParser p, final String raw)
	{
		...
		try
		{
			...
			final byte[] token = incoming == null ? firstToken()
					: nextToken(incoming);
			String encodedToken = DatatypeConverter.printBase64Binary(token);
			encodedToken = (encodedToken != null) ? encodedToken
					: "INVALID_TOKEN";

			response = scheme + " " + encodedToken;
	 ...
	 *             if <code>Negotiator.getSupported()</code> or
	 ...
	 * 
	 * @param token
	 *            the token to be fed into <code>negotiator.nextToken()</code>
	 * @returns the token
	 * @throws java.io.IOException
	 *             if <code>negotiator.nextToken()</code> throws Exception. May
	 *             happen if the input token is invalid.
	 */
...
abstract class Negotiator
{
			...
			throws Exception
	{
		...
		final Class clazz = Class
				.forName("sun.net.www.protocol.http.NegotiatorImpl");
		final java.lang.reflect.Constructor c = clazz.getConstructor(
