 ...
 * <p>
 * A <code>ChunkedInputStream</code> is also <code>Hurryable</code> so it can be
 * hurried to the end of the stream if the bytes are available on the underlying
 * stream.
 */
public class ChunkedInputStream extends InputStream implements Hurryable
{
	...
	/**
	 * The <code>HttpClient</code> that should be notified when the chunked
	 * stream has completed.
	 */
	private final HttpClient hc;

	/**
	 * The <code>MessageHeader</code> that is populated with any optional
	 ...
	 * space available.
	 */
	private void ensureRawAvailable(final int size)
	{
		if (rawCount + size > rawData.length)
		{
			final int used = rawCount - rawPos;
			if (used + size > rawData.length)
			{
				final byte[] tmp = new byte[used + size];
				if (used > 0)
				{
	...
	private int fastRead(byte[] b, int off, int len) throws IOException
	{
		...
		final int remaining = chunkSize - chunkRead;
		final int cnt = (remaining < len) ? remaining : len;
		if (cnt > 0)
		{
			int nread;
			try
		...
		while (state != STATE_DONE)
		{
			 ...
			 */
				case STATE_AWAITING_CHUNK_HEADER:
					/*
					 * Find \n to indicate end of chunk header. If not found
					 * when there is insufficient bytes in the raw buffer to
					...
					/*
					 * Extract the chunk size from the header (ignoring
					 * extensions).
					 */
					final String header = new String(rawData, rawPos, pos
							- rawPos + 1, "US-ASCII");
					...
					{
						chunkSize = Integer
								.parseInt(header.substring(0, i), 16);
					}
					...
					/*
					 * Chunk has been parsed so move rawPos to first byte of
					 * chunk data.
					 */
					rawPos = pos + 1;
					chunkRead = 0;
				...
				case STATE_READING_CHUNK:
					...
					/*
					 * Compute the number of bytes of chunk data available in
					 * the raw buffer.
					 */
					final int copyLen = Math.min(chunkSize - chunkRead,
							rawCount - rawPos);
					...
					{
						final int cnt = chunkCount - chunkPos;
						if (chunkData.length < cnt + copyLen)
						{
							final byte[] tmp = new byte[cnt + copyLen];
							System.arraycopy(chunkData, chunkPos, tmp, 0, cnt);
							chunkData = tmp;
						}
						else
						{
							System.arraycopy(chunkData, chunkPos, chunkData, 0,
									cnt);
						}
						chunkPos = 0;
						chunkCount = cnt;
					 ...
					 * to the read methods.
					 */
					System.arraycopy(rawData, rawPos, chunkData, chunkCount,
							copyLen);
					rawPos += copyLen;
					chunkCount += copyLen;
					chunkRead += copyLen;
					...
					/*
					 * If all the chunk has been copied into chunkData then the
					 * next token should be CRLF.
					 */
				...
				case STATE_AWAITING_TRAILERS:
					 ...
					 * headers.
					 */
					final String trailer = new String(rawData, rawPos, pos
							- rawPos, "US-ASCII");
					i = trailer.indexOf(':');
					if (i == -1)
					{
					...
					final String key = (trailer.substring(0, i)).trim();
					final String value = (trailer.substring(i + 1,
							trailer.length())).trim();
	 ...
	 */
	private int readAhead(final boolean allowBlocking) throws IOException
	{
	 ...
	 *            trailers.
	 */
	public ChunkedInputStream(final InputStream in, final HttpClient hc,
			final MessageHeader responses) throws IOException
	{
	 ...
	 * If the last chunk (and optional trailers) can be read without blocking
	 * then the stream is considered hurried.
	 * <p>
	 * Note that if an error has occured or we can't get to last chunk without
