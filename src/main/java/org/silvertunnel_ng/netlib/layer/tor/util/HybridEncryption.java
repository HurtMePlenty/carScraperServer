...
/**
 * The <code>HybridEncryption</code> class implements the "hybrid encryption"
 * scheme as described in section 0.3 of the main Tor specification
 * (tor-spec.txt).
 */
public class HybridEncryption
{
	 ...
	 * computed as follows:
	 * 
	 * 1. If M is less than PK_ENC_LEN-PK_PAD_LEN (86), pad and encrypt M with
	 * PK. 2. Otherwise, generate a KEY_LEN byte random key K. Let M1 = the
	 * first PK_ENC_LEN-PK_PAD_LEN-KEY_LEN (70) bytes of M, and let M2 = the
	 * rest of M. Pad and encrypt K|M1 with PK. Encrypt M2 with our stream
	 * cipher, using the key K. Concatenate these encrypted values.
	 */
	private final Cipher cipher;

	/**
	 * Create a new <code>HybridEncryption</code> instance which can be used for
	 * performing "hybrid encryption" operations as described in the main Tor
	 * specification (tor-spec.txt).
	 * @throws org.silvertunnel_ng.netlib.layer.tor.util.TorException
	 */
	public HybridEncryption() throws TorException
	{
		try
		{
	...
	/**
	 * Encrypt the entire contents of the byte array <code>data</code> with the
	 * given <code>TorPublicKey</code> according to the "hybrid encryption"
	 * scheme described in the main Tor specification (tor-spec.txt).
	 * 
	 * @param data
	 ...
	 * @throws org.silvertunnel_ng.netlib.layer.tor.util.TorException
	 */
	public final byte[] encrypt(final byte[] data, final PublicKey publicKey, final byte [] secretKey) throws TorException
	{
		...
		// final byte[] c2 = randomKeyCipher.doFinal(data, PK_DATA_LEN_WITH_KEY,
		// data.length - PK_DATA_LEN_WITH_KEY);
	...
	private byte[] encryptSimple(final byte[] data, final PublicKey publicKey) throws TorException
	{
		try
		{
	...
	/**
	 * Decrypt the contents of the byte array <code>data</code> with the given
	 * <code>TorPrivateKey</code> according to the "hybrid encryption" scheme
	 * described in the main Tor specification (tor-spec.txt).
	 * 
	 * @param data
	...
	public byte[] decrypt(final byte[] data, final PrivateKey privateKey) throws TorException
	{
	...
	private byte[] decryptSimple(final byte[] data, final PrivateKey privateKey) throws TorException
	{
		try
		{
