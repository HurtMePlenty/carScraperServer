 ...
 * <p>
 * Using this class is a simpler alternative to using a PipedInputStream and a
 ...
 * have a more complicated API that requires instantiating two classes and
 * connecting them.
 * <p>
...
public class Echo2CircularByteBuffer
{
	...
	private int spaceLeft()
	{
		...
		{
			// any space between the first write
											// and
	...
	private int available()
	{
		...
		{
			// any space between the first read
											// and
	...
	private int marked()
	{
		...
		{
			// any space between the markPosition
											// and
		...
		public int read() throws IOException
		{
			while (true)
			{
				synchronized (Echo2CircularByteBuffer.this)
				{
					...
					final int available = Echo2CircularByteBuffer.this
							.available();
					if (available > 0)
					{
						final int result = buffer[readPosition] & 0xff;
						readPosition++;
			...
			while (true)
			{
				synchronized (Echo2CircularByteBuffer.this)
				{
					...
					if (available > 0)
					{
						...
						final int firstLen = Math.min(length, buffer.length
								- readPosition);
	...
	protected class CircularByteBufferOutputStream extends OutputStream
	{
		...
		{
			// if (outputStreamClosed) throw new
			// IOException("OutputStream has been closed; cannot flush a closed OutputStream.");
			// if (inputStreamClosed) throw new
			// IOException("Buffer closed by inputStream; cannot flush.");
		...
		public void write(byte[] cbuf, int off, int len) throws IOException
		{
			while (len > 0)
			{
				synchronized (Echo2CircularByteBuffer.this)
				{
					...
					{
						throw new IOException(
								"CircularByteBuffer is full; cannot write "
										+ len + " bytes");
					}
					final int realLen = Math.min(len, spaceLeft);
					final int firstLen = Math.min(realLen, buffer.length
							- writePosition);
